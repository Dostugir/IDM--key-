...
class IDMActivator:
    def __init__(self, root):
        ...
        self.root.geometry("400x350")  # Increased height for new button
    ...
    def setup_ui(self):
        ...
        # Reset button
        reset_button = tk.Button(button_frame, text="Reset IDM", 
                               command=self.reset_idm, width=15,
    ...
    def reset_idm(self):
        """Reset IDM to appear as a new installation"""
        ...
        self.status_var.set("Resetting IDM...")
        self.root.update()
        ...
        if not reg_path:
            ...
            self.status_var.set("Reset failed - IDM not found")
            return
        ...
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, reg_path, 0, winreg.KEY_ALL_ACCESS) as key:
                try:
                    winreg.DeleteValue(key, "Serial")
                except:
                    pass
                try:
                    winreg.DeleteValue(key, "FName")
                except:
                    pass
                try:
                    winreg.DeleteValue(key, "LName")
                except:
                    pass
                try:
                    winreg.DeleteValue(key, "Email")
                except:
                    pass
                try:
                    winreg.DeleteValue(key, "ExpireDate")
                except:
                    pass
                
                # Reset trial information
                winreg.SetValueEx(key, "tvfrdt", 0, winreg.REG_SZ, "")
                winreg.SetValueEx(key, "LstCheck", 0, winreg.REG_SZ, "")
                winreg.SetValueEx(key, "CheckUpdtVM", 0, winreg.REG_SZ, "")
                
                # Generate a new unique ID to make IDM think it's a new installation
                unique_id = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(20))
                winreg.SetValueEx(key, "MID", 0, winreg.REG_SZ, unique_id)
                
                # Reset trial counter
                try:
                    winreg.DeleteValue(key, "trialset")
                except:
                    pass
            
            messagebox.showinfo("Success", "IDM has been reset successfully. It will now recognize your system as a new installation.")
            self.status_var.set("IDM reset successfully")
            
            # Generate a new serial for activation
            self.refresh_serial()
            
        except Exception as e:
            messagebox.showerror("Error", f"Reset failed: {str(e)}")
            self.status_var.set("Reset failed - Error occurred")
